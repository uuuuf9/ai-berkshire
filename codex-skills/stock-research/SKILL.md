...
| 策略 | 建议 | 触发条件/价格区间 |
|------|------|-----------------|----------------------|
| 空仓者 | 买入/观望/回避 | 买入价格区间 | 具体可靠的理由（数据锚定+逻辑链+风险对冲） |
| 持仓者 | 加仓/持有/减仓 | 加仓/减仓信号 | 具体可靠的理由（数据锚定+逻辑链+风险对冲） |
| 卖出信号 | - | 明确的卖出条件 | 具体可靠的理由（数据锚定+逻辑链+风险对冲） |
| 加仓信号 | - | 明确的加仓条件 | 具体可靠的理由（数据锚定+逻辑链+风险对冲） |
...
2. **逻辑链**：说明该数据/事实如何支撑该建议（如"估值分位低于历史中枢叠加自由现金流稳健增长，构成安全边际"）。
3. **风险对冲**：点明对应的主要风险或反证（如"但存货增速 35%>收入增速 18%，存在去化风险"）。
4. **可溯源**：理由中的关键事实须标注所依据的缓存文件路径或 `raw/` 原始财报路径。
...
9. **行动建议有据**：3.4 行动建议每条均含具体可溯源理由（数据锚定 + 逻辑链 + 风险对冲），无空泛套话。
...
13. 行动建议（3.4）每条策略必须给出具体、可靠、可溯源的理由（数据锚定 + 逻辑链 + 风险对冲），禁止空泛套话。
